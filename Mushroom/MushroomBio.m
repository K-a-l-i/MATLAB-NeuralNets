...
mushnet=cascadeforwardnet([s1,s2,s3,s4],'trainscg');
%max number of epochs
mushnet.trainParam.epochs=500;
%The number of epochs that the error on the validation set increses
mushnet.trainParam.max_fail=20;
%initiate net
mushnet=init(mushnet);
%train
[mushnet,netstruct]=train(mushnet,p,t);
%name the net and structure
mushnet.userdata='MushroomBio';
mushnetstruct=netstruct;

%--simulating--
%train
%the user is free to get activations on test and all data but we only need
%the training results for choice of architecture
atrain=sim(mushnet,ptrain); 
% %test
...
%instantiate net
mushnet=cascadeforwardnet(best,'trainscg');
%max number of epochs
mushnet.trainParam.epochs=500;
%The number of epochs that the error on the validation set increses
mushnet.trainParam.max_fail=20;
...
[mushnet,netstruct]=train(mushnet,p,t);
%name the net and structure
mushnet.userdata='MushroomBiomass';
mushstructt=netstruct;
...
%final print to user
fprintf('Congradulations! mushnet has been successfully trained, tested and is ready for deployment!\n')
