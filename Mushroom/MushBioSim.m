...
%Benjamin Strelitz
%MushroomBio Sim
%Finding conditions which produce maximum mushroom biomass
%-------------------------------------------------------------------------

%--------------Clearing Workspace and Importing data----------------------
clear;clc;close all
format short
load MushroomBio.mat

%---------------------Variable Setup--------------------------------------
%string vector to be used for graph titles
titles=string({"effect on biomass yield with varying yeast","effect on biomass yield with varying glucose","effect on biomass yield with varying pH","effect on biomass yield with varying temperature"});
...
max_indxs=find(t==Max_bio);
%finding the conditions that produced the maximum biomass
...
diff=((sim_max-Max_bio)/Max_bio)*100;
...
disp('Giving a biomass yield of:')
disp(Max_bio)
%results for simulated data
disp('Best conditions for simulated data were:')
fprintf("yeast: %g\nglucose: %g\nph: %g\ntemperature: %g\n\n\n", Best_cond(1),Best_cond(2),Best_cond(3),Best_cond(4))
disp('Giving a predicted biomass yield of:')
disp(sim_max)
...
if sim_max>Max_bio
    ...
    disp('Giving a predicted biomass yield of:')
    disp(sim_max)
    fprintf("This is an increase of %g percent biomass over the best case for the given data.\nThe results of this study were proudly brought to you by mushnet.\nHave a lovely day!\n", diff)
else
    fprintf("yeast: %g\nglucose: %g\nph: %g\ntemperature: %g\n\n",best_cond(1),best_cond(2),best_cond(3),best_cond(4))
    disp('Giving a biomass yield of:')
    disp(Max_bio)
    ...
    for k=i+1:4
        ...
        for h=1:cellfun('length',stps(k))
            ...
            for p=1:cellfun('length',stps(k))
                ...
                bio_row=[bio_row,mushnet(newcond2)];
            end
            grid=[grid;bio_row];    
        end
        %plotting graph
        ...
        ylabel(xlabs(k));
        zlabel('Biomass');
        title(sprintf('Biomass with respect to %s and %s : (*=maximum)',xlabs(i),xlabs(k)));
        %plotting max as *
        Max=max(max(grid));
        [x,y]=find(grid==Max);
        hold on
