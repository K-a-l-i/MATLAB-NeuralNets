...
%creating cyclic time information from the old 'day number' variable
dnc = cos(2*pi*(Data(:,1)/365));
dns = sin(2*pi*(Data(:,1)/365));
%---------------------Organizing Data-----------------------------------
%creating variables of correct dimensions
p=[dnc,dns,Data(:,2:3)]'; t=Data(:,4)';
...
ionnet=cascadeforwardnet([s1,s2,s3,s4],'trainscg');
%max number of epochs
ionnet.trainParam.epochs=500;
%The number of epochs that the error on the validation set increses
ionnet.trainParam.max_fail=20;
%initiate net
ionnet=init(ionnet);
%train
[ionnet,netstruct]=train(ionnet,p,t);
%name the net and structure
ionnet.userdata='ionnet';
ionstruct=netstruct;
...
%instantiate net
ionnet=cascadeforwardnet(best,'trainscg');
%max number of epochs
ionnet.trainParam.epochs=500;
%The number of epochs that the error on the validation set increses
ionnet.trainParam.max_fail=20;
...
r2=R(1,2)^2;
%printing results
fprintf('Testing Assesment Metrics:\n\n')
fprintf(' corr coeff: %g\n p value: %g\n r2: %g\n',R(1,2),PV(1,2),r2)
disp('----------------------------------------------------------------------')
...
%final print to user
fprintf('Congradulations! ionnet has been successfully trained, tested and is ready for deployment!\nHave a lovely day!\n')
